...
# Chemin : res://scripts/gameplay/gardien.gd
# Scène  : res://scenes/gameplay/gardien.tscn
#
...
@onready var zone_detect:     Area2D          = $ZoneDetection2
@onready var label_interagir: Label           = $LabelInteragir2
@onready var dessin:          AnimatedSprite2D = $GardienDessin2
...
var _sens_patrouille: int   = 0   # 1 = vers la droite, -1 = vers la gauche
...
func _physics_process(delta: float) -> void:
	...
	if dialogue_joue and DialogueManager.en_cours or joueur_dans_zone:
		...
		(dialogue_joue and DialogueManager.en_cours) or joueur_dans_zone
		move_and_slide()
